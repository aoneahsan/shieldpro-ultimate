        ...
        {Object.keys(testResults).length > 0 && (
          <div className="mt-2 text-sm">
            <span className="text-purple-700">
              Matches: {Object.values(testResults).filter(r => r).length} / {Object.keys(testResults).length}
            </span>
          </div>
        )}
      </div>
            ...
            {newPattern.pattern && !patternError && (
              <div className="mt-2 flex items-center space-x-4 text-sm">
                <span className={`flex items-center space-x-1 ${
                  getPerformanceRating(calculateComplexity(newPattern.pattern)) === 'fast' ? 'text-green-600' :
                  getPerformanceRating(calculateComplexity(newPattern.pattern)) === 'medium' ? 'text-yellow-600' :
                  'text-red-600'
                }`}>
                  <Zap className="w-4 h-4" />
                  <span>Performance: {getPerformanceRating(calculateComplexity(newPattern.pattern))}</span>
                </span>
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2 py-0.5 text-xs rounded ${
                      pattern.category === 'url' ? 'bg-blue-100 text-blue-700' :
                      pattern.category === 'domain' ? 'bg-green-100 text-green-700' :
                      pattern.category === 'element' ? 'bg-purple-100 text-purple-700' :
                      pattern.category === 'content' ? 'bg-yellow-100 text-yellow-700' :
                      'bg-gray-100 text-gray-700'
                    }`}>
                      {pattern.category}
                    </span>
                    <span className={`px-2 py-0.5 text-xs rounded ${
                      pattern.action === 'block' ? 'bg-red-100 text-red-700' :
                      pattern.action === 'allow' ? 'bg-green-100 text-green-700' :
                      pattern.action === 'modify' ? 'bg-orange-100 text-orange-700' :
                      'bg-blue-100 text-blue-700'
                    }`}>
                      {pattern.action}
                    </span>
                    <span className={`px-2 py-0.5 text-xs rounded flex items-center space-x-1 ${
                      pattern.performance === 'fast' ? 'bg-green-100 text-green-700' :
                      pattern.performance === 'medium' ? 'bg-yellow-100 text-yellow-700' :
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {Math.round(patterns.reduce((sum, p) => sum + p.complexity, 0) / Math.max(patterns.length, 1))}
            </div>

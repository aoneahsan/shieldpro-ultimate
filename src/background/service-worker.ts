      ...
      case 'adBlocked':
        ...
        if (sender.tab?.id) {
          ...
          await storage.incrementBlockedCount(
            domain,
            category as keyof BlockingStats['categoryStats']
          );
        }
        sendResponse({ success: true });
        break;
...
function categorizeRequest(
  url: string
): keyof import('../shared/types').BlockingStats['categoryStats'] {
  const lowerUrl = url.toLowerCase();

  // YouTube specific
  if (
    lowerUrl.includes('youtube.com') &&
    (lowerUrl.includes('/api/stats/') ||
  ...
  if (
    ...
    lowerUrl.includes('hotjar') ||
    lowerUrl.includes('fullstory') ||
    lowerUrl.includes('mouseflow') ||
    lowerUrl.includes('clarity.ms') ||
    lowerUrl.includes('crazyegg')
  ) {
    return 'trackers';
  }

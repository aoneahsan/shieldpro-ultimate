...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'SIGN_IN_WITH_GOOGLE') {
    handleGoogleSignIn().then(sendResponse).catch(error => {
  ...
  if (request.type === 'SIGN_IN_WITH_EMAIL') {
    handleEmailSignIn(request.email, request.password).then(sendResponse).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  ...
  if (request.type === 'RESET_PASSWORD') {
    handlePasswordReset(request.email).then(sendResponse).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  ...
  if (request.type === 'SIGN_OUT') {
    handleSignOut().then(sendResponse).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
  ...
  if (request.type === 'GET_CURRENT_USER') {
    handleGetCurrentUser().then(sendResponse).catch(error => {
      sendResponse({ success: false, error: error.message });
    });
    return true;
  }
...
async function handleGoogleSignIn() {
  try {
    ...
    const clientId = process.env.FIREBASE_CLIENT_ID || '526899927330-q60p30m9tjt8nb9av2bgq7tii388kfgf.apps.googleusercontent.com';
